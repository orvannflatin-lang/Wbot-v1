...
// console.clear(); // Commented out to see logs
console.log('╭──────────────────────────────────╮');
...
async function startSocket(usePairing, phoneNumber) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (qr && !usePairing) {
            console.log('\n📷 QR Code généré...');
            // Dynamic import to avoid errors if missing, though it is in package.json
...
async function main() {
    console.log('🚀 Démarrage automatique en mode QR Code...');
    startSocket(false, '');
}
